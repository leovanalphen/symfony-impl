...
// runningInfo is a JSON-safe projection of orchestrator.RunningEntry, omitting
// fields that cannot be marshalled (e.g. CancelFn).
...
func toRunningInfo(e *orchestrator.RunningEntry) runningInfo {
	return runningInfo{
		IssueID:         e.IssueID,
...
func (s *Server) handleState(w http.ResponseWriter, r *http.Request) {
	...
	for _, e := range snap.Running {
		running = append(running, toRunningInfo(e))
	}
...
`, len(snap.Running))
	for _, e := range snap.Running {
		fmt.Fprintf(w, "<li>%s - %s - turns: %d - tokens: %d</li>\n",
			html.EscapeString(e.IssueIdentifier),
			html.EscapeString(e.LastEvent),
			e.TurnCount,
			e.TotalTokens)
	}
	fmt.Fprintf(w, `</ul>
...
`, len(snap.Retries))
	for _, r := range snap.Retries {
		fmt.Fprintf(w, "<li>%s - attempt %d - due: %d</li>\n",
			html.EscapeString(r.Identifier),
			r.Attempt,
			r.DueAtMs)
	}
	fmt.Fprintf(w, `</ul>
